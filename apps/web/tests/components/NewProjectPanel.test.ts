...
describe('NewProjectPanel image provider visibility', () => {
  it('shows Vela image models without changing the HyperFrames video surface', () => {
    ...
    expect(videoModels.some((model) => model.provider === 'hyperframes')).toBe(true);
    expect(videoModels.some((model) => model.provider === 'vela')).toBe(false);
