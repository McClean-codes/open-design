...
describe('project resource selection uses the persisted exact member', () => {
  ...
  it('does not create a project when the exact Team plugin is tombstoned during registry loading', async () => {
    const insertProject = vi.fn((_: unknown, input: Record<string, unknown>) => input);
    const insertConversation = vi.fn();
    const source = `team:plugin:${WORKSPACE_ID}:shared-id`;
    let bindingLive = true;
    let finishRegistryLoad!: (value: {
      skills: never[];
      designSystems: never[];
      craft: never[];
      atoms: never[];
    ...
    const registryGate = new Promise<{
      skills: never[];
      designSystems: never[];
      craft: never[];
      atoms: never[];
    ...
    const getLocalPluginBySource = vi.fn(async () => bindingLive
      ? { id: 'shared-id', source }
      : null);
    const loadRegistry = vi.fn(async () => {
      registryLoadStarted();
    ...
    const responsePromise = fetch(`${baseUrl}/api/projects`, {
      ...
      headers: headers(),
      body: JSON.stringify({
        id: 'tombstoned-during-create',
        name: 'Tombstoned during create',
        pluginId: 'shared-id',
        pluginSource: source,
      }),
    });
