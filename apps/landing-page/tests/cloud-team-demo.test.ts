...
test('cloud team upgrade demo leads the benefit list with the campaign entitlement', () => {
  ...
  assert.match(source, /class="campaign-header-banner"/);
  assert.match(source, /class="campaign-header-badge">无限使用/);
  assert.match(source, /这次，别省着用。DeepSeek V4 Flash 无限用。/);
  assert.match(source, /8 月 6 日 20:00—8 月 13 日 20:00，一周免费用/);
  assert.match(source, /data-campaign-countdown/);
  assert.match(source, />7天 00:00:00<\/strong>/);
  assert.match(source, /campaignCountdown\.textContent = `\$\{days\}天/);
  assert.match(source, /campaignCountdownEndsAt = Date\.now\(\) \+ 7 \* 24 \* 60 \* 60 \* 1000/);
  assert.match(source, /\.campaign-header-banner \{[^}]*border:\s*1px solid/);
  assert.match(source, /class="plan-campaign-benefit"/);
  assert.match(source, /campaignEligible: false/);
  ...
  assert.doesNotMatch(source, /\.campaign-header-disclaimer strong/);
  assert.ok(
...
test('cloud team upgrade demo exposes personal and team pricing views', () => {
  ...
  assert.match(source, /\.personal-plan-grid \{ grid-template-columns: repeat\(4, 1fr\); \}/);
