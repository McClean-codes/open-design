...
const EN: PricingContent = {
  labels: {
    ...
    multimodalBenefit: 'Top models, ready to use for agents, images, and video',
    multimodalTitle: 'One credit balance powers agents and multimodal creation',
    multimodalDescription: 'From understanding a brief and executing design work to generating images and video—without configuring provider API keys. See an estimate before generation; successful generations are charged by actual usage.',
...
const ZH_CN: PricingContent = {
  labels: {
    ...
    multimodalBenefit: '顶级模型开箱即用，覆盖 Agent、图片与视频创作',
    multimodalTitle: '一份模型额度，驱动 Agent 与多模态创作',
    multimodalDescription: '从理解需求、规划并执行设计任务，到生成图片和视频，无需分别配置供应商 API Key。生成前展示预估费用，成功后按实际用量扣除。',
...
const ZH_TW: PricingContent = {
  labels: {
    ...
    multimodalBenefit: '頂級模型開箱即用，涵蓋 Agent、圖片與影片創作',
    multimodalTitle: '一份模型額度，驅動 Agent 與多模態創作',
    multimodalDescription: '從理解需求、規劃並執行設計任務，到生成圖片與影片，無需分別配置供應商 API Key。生成前顯示預估費用，成功後依實際用量扣除。',
...
const ES: PricingContent = {
  labels: {
    ...
    multimodalBenefit: 'Modelos de primer nivel listos para agentes, imágenes y vídeo',
    multimodalTitle: 'Un saldo impulsa agentes y creación multimodal',
    multimodalDescription: 'Desde comprender el encargo y ejecutar el trabajo de diseño hasta generar imágenes y vídeo, sin configurar claves API de proveedores. Consulta una estimación antes de generar; solo se cobra el uso real de las generaciones completadas.',
...
const PT_BR: PricingContent = {
  labels: {
    ...
    multimodalBenefit: 'Modelos de ponta prontos para agentes, imagens e vídeos',
    multimodalTitle: 'Um saldo impulsiona agentes e criação multimodal',
    multimodalDescription: 'Da compreensão do briefing e execução do trabalho de design à geração de imagens e vídeo, sem configurar chaves de API de provedores. Veja uma estimativa antes de gerar; gerações concluídas são cobradas pelo uso real.',
...
const RU: PricingContent = {
  labels: {
    ...
    multimodalBenefit: 'Лучшие модели сразу готовы для агентов, изображений и видео',
    multimodalTitle: 'Единый баланс для агентов и мультимодального творчества',
    multimodalDescription: 'От понимания задачи и выполнения дизайн-работы до генерации изображений и видео — без настройки API-ключей провайдеров. До генерации показывается оценка, а успешные генерации оплачиваются по фактическому использованию.',
...
const FR: PricingContent = {
  labels: {
    ...
    multimodalBenefit: 'Des modèles de pointe prêts pour les agents, l’image et la vidéo',
    multimodalTitle: 'Un seul solde pour les agents et la création multimodale',
    multimodalDescription: 'De la compréhension du brief à l’exécution du travail de design, puis à la génération d’images et de vidéos, sans configurer de clés API fournisseur. Une estimation s’affiche avant la génération ; les générations réussies sont facturées selon l’usage réel.',
...
const KO: PricingContent = {
  labels: {
    ...
    multimodalBenefit: '최상급 모델로 Agent·이미지·영상 제작을 바로 시작',
    multimodalTitle: '하나의 크레딧으로 Agent와 멀티모달 창작',
    multimodalDescription: '요구사항을 이해하고 디자인 작업을 계획·실행하는 것부터 이미지와 동영상 생성까지, 공급자 API 키를 별도로 설정할 필요가 없습니다. 생성 전 예상 비용을 확인하고, 성공한 생성은 실제 사용량만큼 차감됩니다.',
...
const DE: PricingContent = {
  labels: {
    ...
    multimodalBenefit: 'Top-Modelle sofort einsatzbereit für Agenten, Bilder und Videos',
    multimodalTitle: 'Ein Guthaben für Agenten und multimodale Kreation',
    multimodalDescription: 'Vom Verstehen des Briefings und Ausführen der Designarbeit bis zur Bild- und Videogenerierung — ohne separate Anbieter-API-Schlüssel. Vor der Generierung erscheint eine Schätzung; erfolgreiche Generierungen werden nach tatsächlicher Nutzung abgerechnet.',
...
const JA: PricingContent = {
  labels: {
    ...
    multimodalBenefit: 'トップモデルをすぐに利用し、Agent・画像・動画を制作',
    multimodalTitle: '1つのクレジットで Agent とマルチモーダル制作',
    multimodalDescription: '要件の理解、デザイン作業の計画・実行から画像と動画の生成まで、プロバイダーの API キーを個別に設定する必要はありません。生成前に見積もりを表示し、成功した生成は実際の使用量に応じて課金されます。',
