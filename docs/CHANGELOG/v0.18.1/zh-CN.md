...
- ☁️ **不再动不动就崩的云端登录。** *打包客户端可能陷进 502 循环——一个失败的云端请求被高频心跳反复重试，直到整个工作区看起来像坏了。* 0.18.1 让心跳退避，把服务端真正说的话透传出来而不是丢一个笼统的失败，并对值得重试的代理请求做重放。云端抖动现在看起来只是糟糕的一分钟，而不是装坏了的应用。 (#6521)

- ⚡ **秒开的设计文件。** *文件一多，网格会一次性渲染全部内容并把每张缩略图都提前拉下来。* 现在网格只渲染你看得见的部分，缩略图随滚动按需加载——大项目的打开时间降到原来的零头。 (#6513)

- 🔑 **你自己的模型，回到选择器里了。** *BYOK 模型从首页和输入框的选择器里消失了——明明配好了服务商，却切不过去。* 已修复：你接入的每个模型都能重新选中，两处都可以。 (#6515)
